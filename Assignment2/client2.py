...
class MyGetOperationServicer(get_operation_pb2.GetOperationServicer):
    def __init__(self):
        self.db = rocksdb.DB("client2.db", rocksdb.Options(create_if_missing=True))

    #this method makes modifications to the DB based on the received operation
    def addToClientDB(self,request,context):
        #here we receive the operation along with KEY/VALUE data depending on the operation
        #we tokenize the reponse and perform the operation on local rocksdb
        str = get_operation_pb2.ResponseOfOperation()
        x = request.data
        str.data = x + " received"
        print(str)
        status = datastore_pb2.Response()
        status.data = x + " added to db"
        return status

#we create 'server1' for this client to expose a service 'addToClientDB'
server1 = grpc.server (futures.ThreadPoolExecutor (max_workers=5))
get_operation_pb2_grpc.add_GetOperationServicer_to_server(MyGetOperationServicer(), server1)
server1.add_insecure_port('%s:%d' % ('0.0.0.0', CLIENT_PORT))
server1.start()
...
print('Clients IP address: %s' %(host1))
#args.host contains the IP address of the server
print('Client is connecting to Server at %s:%d...' %(args.host, SERVER_PORT))
